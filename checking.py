                        ...
                        if len(container_part) in containersSPR_table[platform[1]].keys():
                            for codes in containersSPR_table[platform[1]][len(container_part)]:
                                if len(container_part) == len(codes):
                                    flag = True
                                    for d_class, allowed_classes in zip(
                                            (np.array(container_part, dtype=object)[:, 8]).astype(str),
                                            np.array(codes, dtype=object)[:, 2]):
                                        if d_class not in allowed_classes and d_class != "":
                                            flag = False
                                    if (((np.array(container_part, dtype=object)[:, 2]).astype(str) == (
                                            ...
                                            and ((np.array(container_part, dtype=object)[:, 4]).astype(str) == (
                                                    np.array(codes, dtype=object)[:, 6]).astype(str)).all() and flag):
                            ...
                            if len(correct_codes) > 0:
                                placed_containers[platform[1]].append([correct_codes, container_part])
                                placed_containers_for_checking.append(container_part)
        final_result[self.sessionID] = placed_containers
        return final_result
