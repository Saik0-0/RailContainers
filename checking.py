                ...
                if platform[1] in list(containersSPR_table.keys()):
                    for container_part in session[1]:
                            ...
                            if str(code) in correct_codes:
                                ...
                                for index, row in enumerate(rule_code):
                                    if row[0] == 'Масса':
                                        ...
                                        weight_diff = abs(
                                            container_part_np[row[1][0] - 1, 3] - container_part_np[row[1][1] - 1, 3])
                                        if row[2] == 'Меньше равно':
                            ...
                            if str(code) in correct_codes:
                                ...
                                for index, condition in enumerate(table_conditions):
                                    if condition[0][0] == 'Масса':
                                        if (condition[0][2] <= container_part_np[condition[0][1][0] - 1, 3] <=
                                                condition[0][3]
                                                and condition[1][2] <= container_part_np[condition[1][1][0] - 1, 3] <=
                                                condition[1][3]):
                                            flags[index] = True
                                        ...
                                        if condition[0][2] <= weight_sum <= condition[0][3] and condition[1][
                                            2] <= weight_diff <= condition[1][3]:
                                            flags[index] = True
                                        ...
                                        for ind in condition[0][1]:
                                            weight_diff += container_part_np[ind - 1, 3]
                                        if condition[1][2] <= weight_sum <= condition[1][3] and condition[0][
                                            2] <= weight_diff <= condition[0][3]:
                                            flags[index] = True
                        ...
                        if len(correct_codes) > 0:
                            print(
                                f'Для сессии {sessionID}, для платформы {platform} подошло разбиение контейнеров {container_part} по коду {correct_codes}')
